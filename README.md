...
원래는 슬라이딩 윈도우를 2초 간격으로 굴렸는데, 라즈베리파이 CPU 온도가 75도까지 올라가는 걸 보고 4초 간격으로 바꿨습니다. 프리필터가 대부분 구간을 미리 걸러주기도 해서 4초로도 실시간성에는 큰 차이가 없습니다.

## 알림 정책

현장에 배포하면서 차장님이랑 협의해서 정한 규칙입니다.
...
용량 때문에 Git에 넣지 않았습니다. Releases 페이지에서 받아서 프로젝트 루트에 그대로 두면 됩니다.
...
YAMNet hub 모델까지 쓰려면 다음 한 줄을 추가로 받아두세요.
